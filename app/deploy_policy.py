...
_V4_LANDING: Final = LandingConditions(
    ...
    rationale=(
        ...
        "factory. What follows is that the lane these terms govern cannot land a factory pull "
        "request at all: an update type is read from a pull request title stating a single "
        "version delta, a factory title states none, and no rule about update types applies to "
        "one -- the same answer, by the same route, that a requirement-range bump already gets. "
        "That is correct rather than a gap. A factory pull request is landed by the lane that "
        "adjudicated the work behind it, which asks this service whether the change was routed "
        "and approved and then applies its own conditions on the act."
    ),
    rollout_workflows={
        "alobarquest/change-manager": WorkflowPin(
            path=".github/workflows/deploy.yml",
