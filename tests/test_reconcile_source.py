...
def sec_esc(uuid="fp1", check="shell.plaintext_secret", urgent=True):
    return EscalationIn(
        ...
        plan={"tier": "URGENT", "source": "security"},
        note=None,
        urgent=urgent,
    )
...
def drift_req(escalations):
    return SyncRequest(
        ...
        source_report="2026-06-15.remediation.json",
...
def sec_req(escalations):
    return SyncRequest(
        generated_at="2026-06-15T03:00:00Z",
        source_report="2026-06-15.security.json",
        escalations=escalations,
        source="security",
    )
...
def test_source_defaults_to_drift_for_legacy_payloads(db):
    ...
    legacy = SyncRequest(
        generated_at="2026-06-15T03:00:00Z", source_report="x.json", escalations=[drift_esc()]
    )
...
def rot_req(escalations):
    return SyncRequest(
        generated_at="2026-06-18T03:00:00Z",
        source_report="rotation-scan-2026-06-18.json",
        escalations=escalations,
        source="rotation",
    )
